...
public class DisplayTextNodeV3 : MessageNode
{
	...
	public override string GetID => "DisplayTextNodeV3";
	...
	{
		DisplayTextNodeV3 obj = (DisplayTextNodeV3)Create(rect.position + new Vector2(20f, 20f));
		obj._text = _text;
		obj._character = _character;
		obj._item = _item;
		obj._priority = _priority;
		obj._groupId = _groupId;
		return obj;
	}
	...
	public override void Execute(NodeCanvas canvas)
	{
		...
		GetInputValue(Inputs[4], ref _character, canvas);
		GetInputValue(Inputs[5], ref _item, canvas);
		GetInputValue(Inputs[3], ref _groupId, canvas);
		List<int> inputValue = GetInputValue<List<int>>(Inputs[6], canvas);
		List<LocalizedString> inputValue2 = GetInputValue<List<LocalizedString>>(Inputs[7], canvas);
		TextJournalContent textJournalContent = new TextJournalContent(_text, _priority);
		if (_groupId != null)
		{
			textJournalContent.GroupId = _groupId;
		}
		if (_character != null)
		{
